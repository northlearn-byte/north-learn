...
import { LANGUAGES, WORD_DICTIONARY, translateWord } from '../data/stories';
import { 
  ArrowRightLeft, 
  Volume2, 
  Copy, 
...
export const Translator = () => {
  ...
  const handleTranslate = () => {
    ...
    const analysis = words.map(word => {
      ...
      // Fallback/cleanup translator string format
    ...
    // Build sentence translation
    // If the sentence is a common one, translate it. Otherwise combine word translations.
    let fullTranslation = '';
    
    // Check if it's a known sentence in stories or dictionary, or construct simple translation
    const joined = sourceText.trim().toLowerCase();
    
    // Simple mock translation for common tutorial/learning sentences
    if (joined.includes("hello") && joined.includes("world")) {
      fullTranslation = destLang === 'ar' ? 'مرحباً بالالعالم' : 'Hola Mundo';
    } else if (joined.includes("how are you")) {
      fullTranslation = destLang === 'ar' ? 'كيف حالك؟' : '¿Cómo estás?';
    } else if (joined.includes("good morning")) {
      fullTranslation = destLang === 'ar' ? 'صباح الخير' : 'Buenos días';
    } else {
      // Build phrase translation using the local dictionary lookup
      fullTranslation = analysis
        .map(a => a.translation)
        .filter(t => t !== `(${t('Words')})`)
        .join(' ');
      
      if (!fullTranslation) {
        fullTranslation = destLang === 'ar' 
          ? `ترجمة تقريبية: ${sourceText}`
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      handleTranslate();
    }, 4000); // Translate after 4s of inactivity
  ...
  const handleSpeak = (text, langCode) => {
    const speakLang = langCode === 'en' ? 'en-US' : (langCode === 'ar' ? 'ar-SA' : 'es-ES');
      ...
      <div className="relative rounded-3xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-8 sm:p-12 text-white shadow-2xl overflow-hidden mb-8">
        ...
        <div className="relative z-10 max-w-2xl">
          <span className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-extrabold uppercase tracking-wider mb-4 border border-white/30">
            <Sparkles className="w-3.5 h-3.5 text-amber-300" /> {t('Translator 🌐')}
          </span>
          ...
          <div className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg">
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-slate-100 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2 font-bold text-sm text-slate-700 dark:text-slate-200">
                <span className="text-lg">{sourceLang === 'en' ? '🇺🇸' : sourceLangObj.flag}</span>
                <span>{sourceLang === 'en' ? 'English' : sourceLangObj.name}</span>
              </div>
              ...
              <div className="flex items-center gap-2 font-bold text-sm text-slate-700 dark:text-slate-200">
                <span className="text-lg">{destLangObj.flag}</span>
                <span>{destLangObj.name}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
              ...
              <div className="p-5 flex flex-col justify-between min-h-[220px] bg-slate-50/50 dark:bg-slate-950/20">
                <div className="w-full min-h-[160px] text-slate-900 dark:text-slate-50 text-base font-semibold leading-relaxed whitespace-pre-wrap select-all">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800/50">
                  ...
                  <button 
                    ...
                    className="px-4 py-1.5 rounded-lg bg-sky-600 text-white font-bold text-xs hover:bg-sky-700 shadow-md transition-all active:scale-95"
                  >
                    Translate
                  </button>
                </div>
              ...
              {history.length === 0 ? (
                ...
                <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
                  {history.map((item) => (
                    <div 
                      key={item.id} 
                      onClick={() => { setSourceText(item.sourceText); setTranslatedText(item.translatedText); }}
                    ...
                    >
                      <div className="flex items-center justify-between text-[10px] text-slate-400 mb-1 font-bold">
                        <span>🇺🇸 English → {LANGUAGES.find(l => l.code === item.destLang)?.flag} {LANGUAGES.find(l => l.code === item.destLang)?.name}</span>
                      </div>
